...
class UpSample(nn.Module):
    def __init__(self, channel, outchannel=0):
        super(UpSample, self).__init__()
        # 插值之后有一个卷积，1*1，步长是1，只是为了降通道
        if outchannel==0:
            outchannel=channel//2
        self.layer = nn.Conv2d(channel, channel//2, 1, 1)
        self.layer1=nn.ConvTranspose2d(channel,outchannel,3,2,1,1)
...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.c1 = Conv_Block(3, 64) #q 64x256
        self.d1 = DownSample(64)#q 64x128
        self.c2 = Conv_Block(64, 128)# q 128x128
        self.d2 = DownSample(128)# q128x64
        self.c3 = Conv_Block(128, 256) #q256x64
        self.d3 = DownSample(256)#q256x32
        self.c4 = Conv_Block(256, 512)#q512x32
        self.d4 = DownSample(512)#q512x16
        self.c5 = Conv_Block(512, 1024)#q1024x16
        ...
        self.out = nn.Conv2d(64, 21, 3, 1, 1)  # 3个通道代表RGB彩色
        self.Th = nn.Softmax(dim=1) # 分割只需要区分0（无颜色）和1（彩色）
